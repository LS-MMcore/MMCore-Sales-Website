...
// Static list; names remain literal. Region/type get translated.
const carrierList = [
...
const typeKey = (type: string) => toKey(type)
const regionKey = (region: string) => {
  const normalized = region.trim().toLowerCase()
  if (normalized === "united kingdom") return "unitedKingdom"
  return toKey(region)
}
...
export default function TMSPage() {
  ...
  const tList = (key: string): string[] => {
    const res = (t as any)(key, { returnObjects: true }) as unknown
  ...
  const advantages = useMemo(
    () => ({
      operational: tList("services.keyAdvantages.lists.operational"),
      financial: tList("services.keyAdvantages.lists.financial"),
      sales: tList("services.keyAdvantages.lists.sales"),
    }),
    [t],
  )
  ...
  const platformFeatures = [
    {
      ...
      benefits: [
        t("services.platformCapabilities.apiIntegration.contractTerm"),
        t("services.platformCapabilities.apiIntegration.activation"),
        t("services.platformCapabilities.apiIntegration.apiConnection"),
      ],
    },
    {
      icon: Zap,
      ...
      benefits: [
        t("services.platformCapabilities.customsSupport.webshopPlugins"),
        t("services.platformCapabilities.customsSupport.customFrontend"),
        t("services.platformCapabilities.customsSupport.automatedDataExchange"),
      ],
    },
    {
      icon: Shield,
      ...
      benefits: [
        t("services.platformCapabilities.billingReconciliation.crossBorderShipping"),
        t("services.platformCapabilities.billingReconciliation.legalCompliance"),
        t("services.platformCapabilities.billingReconciliation.automatedDataDeletion"),
      ],
    },
    {
      icon: BarChart3,
      ...
      benefits: [
        t("services.platformCapabilities.cloudBased.invoiceVerification"),
        t("services.platformCapabilities.cloudBased.billingErrorDetection"),
        t("services.platformCapabilities.cloudBased.costManagement"),
      ],
    },
  ]
  ...
  const carrierStats = [
    ...
    {
      icon: MapPin,
      label: t("services.carrierNetwork.stats.injectionPointsLabel"),
      value: t("services.carrierNetwork.stats.injectionPoints"),
    },
    {
      icon: Clock,
            ...
            <div className="text-center space-y-4 max-w-4xl mx-auto">
              <div className="flex justify-center mb-6">
                <div className="p-3 rounded-full" style={{ backgroundColor: "#63b2dc20" }}>
                  <Truck className="h-8 w-8" style={{ color: "#63b2dc" }} />
                </div>
              </div>
              ...
              <div className="flex flex-wrap justify-center gap-2 mt-6">
                <Badge variant="default" className="text-sm" style={{ backgroundColor: "#63b2dc" }}>
                  {t("services.badge.carriers")}
                </Badge>
                <Badge variant="default" className="text-sm" style={{ backgroundColor: "#63b2dc" }}>
                  {t("services.badge.countries")}
                </Badge>
                <Badge variant="default" className="text-sm" style={{ backgroundColor: "#63b2dc" }}>
                  {t("services.badge.cloud")}
                </Badge>
                <Badge variant="default" className="text-sm" style={{ backgroundColor: "#63b2dc" }}>
                  {t("services.badge.gdpr")}
                </Badge>
              </div>
            </div>
          </div>
          ...
          <div className="container px-4 md:px-6">
            <div className="text-center space-y-4 mb-12">
              ...
              <p className="mx-auto max-w-[700px] text-slate-600 md:text-lg">
                {t("services.platformCapabilities.description")}
              </p>
            </div>
        ...
        <section className="w-full py-12 md:py-24 bg-white">
          <div className="container px-4 md:px-6">
            <div className="text-center space-y-4 mb-12">
              ...
              <p className="mx-auto max-w-[700px] text-slate-600 md:text-lg">{t("services.ctaParagraph")}</p>
            </div>
        ...
        >
          <div className="container px-4 md:px-6">
            ...
            <Card className="max-w-6xl mx-auto bg-white/95 backdrop-blur-sm">
              <CardHeader>
                <CardTitle className="text-slate-800">{t("services.carrierNetwork.section.title")}</CardTitle>
                <CardDescription className="text-slate-600">
                  {t("services.carrierNetwork.section.subtitle")}
                </CardDescription>
              </CardHeader>
              <CardContent>
                    ...
                    >
                      <div className="flex flex-col">
                        ...
                        <span className="text-sm text-slate-600">
                          {t(`services.carrierNetwork.regions.${regionKey(carrier.region)}`)}
                        </span>
                      </div>
                      <Badge variant="outline" className="text-xs" style={{ borderColor: "#63b2dc", color: "#63b2dc" }}>
                        {t(`services.carrierNetwork.types.${typeKey(carrier.type)}`)}
                      </Badge>
                    </div>
                  ))}
                  <div
        ...
        <section className="w-full py-12 md:py-24" style={{ backgroundColor: "#63b2dc10" }}>
          <div className="container px-4 md:px-6">
            <div className="text-center space-y-6 max-w-2xl mx-auto">
              ...
              <p className="text-slate-700 md:text-lg">{t("tms.cta.subtitle")}</p>
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                ...
                >
                  <Link href="/api-documentation">{t("services.apiDocumentation")}</Link>
                </Button>
              </div>
            </div>
          </div>
